...
var Lazy_RightMenu = {
	...
	_createItem : function(isSplit, option) {
		...
		if (isSplit) {
			...
			menuItem = $("<div class='menu_item'>" + "<p class='item_content' name='li_" + option.optParam + "' onclick='javascript:"
					+ "Lazy_RightMenu.optFunc(" + JSON.stringify(option.optParam) + "," + JSON.stringify(this.model)
					+ ")' >" + option.optName + "</p>" + "<p class='more_menus'></p>" + "</div>");
		}
		return menuItem;
	},
